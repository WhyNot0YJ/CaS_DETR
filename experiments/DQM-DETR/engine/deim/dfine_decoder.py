...
__all__ = ['DFINETransformer', 'DQMDFINETransformer']
...
class DegradationQueryModulator(nn.Module):
    def __init__(self, hidden_dim=256, dqm_hidden_dim=128, dqm_num_heads=4, dqm_dropout=0.0, act='relu'):
        super().__init__()
        assert hidden_dim % dqm_num_heads == 0, "hidden_dim must be divisible by dqm_num_heads"
        self.degradation_token = nn.Parameter(torch.zeros(1, 1, hidden_dim))
        self.degradation_attn = nn.MultiheadAttention(
            hidden_dim,
            dqm_num_heads,
            dropout=dqm_dropout,
            batch_first=True,
        ...
        self.state_head = nn.Sequential(
            nn.Linear(hidden_dim, dqm_hidden_dim),
            get_activation(act),
            nn.Linear(dqm_hidden_dim, dqm_hidden_dim),
            get_activation(act),
        )
        self.query_head = nn.Linear(dqm_hidden_dim, hidden_dim * 2)
        init.constant_(self.query_head.weight, 0)
        init.constant_(self.query_head.bias, 0)

    def forward(self, memory):
        token = self.degradation_token.expand(memory.shape[0], -1, -1)
        token, _ = self.degradation_attn(token, memory, memory, need_weights=False)
        z_d = self.state_head(token.squeeze(1))
        gamma, beta = self.query_head(z_d).chunk(2, dim=-1)
...
class DQMDFINETransformer(DFINETransformer):
    ...
    def _apply_query_dqm(self, content, gamma, beta, dn_meta=None, dqm_branch='degraded'):
        ...
        split = 0
        if dn_meta is not None:
            split = int(dn_meta['dn_num_split'][0])

        query = content[:, split:]
        query = query + strength * (query * gamma + beta)
        if split > 0:
    ...
    def forward(self, feats, targets=None, dqm_branch='degraded'):
        ...
        decoder_outputs = self.decoder(
            ...
            attn_mask=attn_mask,
            dn_meta=dn_meta,
            return_query=self.training)

        if self.training:
            out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits, out_queries = decoder_outputs
        else:
            out_bboxes, out_logits, out_corners, out_refs, pre_bboxes, pre_logits = decoder_outputs
        ...
        if self.training:
            out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1], 'pred_corners': out_corners[-1],
                   'ref_points': out_refs[-1], 'up': self.up, 'reg_scale': self.reg_scale,
                   'query_embed': out_queries[-1], 'degradation_state': degradation_state}
        else:
            out = {'pred_logits': out_logits[-1], 'pred_boxes': out_bboxes[-1]}
