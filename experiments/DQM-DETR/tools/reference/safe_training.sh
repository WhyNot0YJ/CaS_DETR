...
TRAIN_CMD="CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --master_port=7777 --nproc_per_node=4 train.py -c $CONFIG_FILE --seed=0 --output-dir $OUTPUT_DIR"
...
if [ $? -ne 0 ]; then
    ...
    while true; do
        RESUME_CMD="CUDA_VISIBLE_DEVICES=0,1,2,3 torchrun --master_port=7777 --nproc_per_node=4 train.py -c $CONFIG_FILE --seed=0 --output-dir $OUTPUT_DIR -r ${OUTPUT_DIR}/last.pth"
        if [ "$SAVE_LOGS" = true ]; then
