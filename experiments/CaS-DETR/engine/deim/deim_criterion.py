...
from .box_ops import box_cxcywh_to_xyxy, paired_box_iou_xyxy, paired_generalized_box_iou_xyxy
...
class DEIMCriterion(nn.Module):
    ...
    def __init__(self, \
        ...
        mal_alpha=None,
        use_uni_set=True,
        decoder_moe_balance_weight=0.0,
        moe_balance_warmup_epochs=0,
        cass_loss_weight=0.0,
        ):
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes, boxes_weight=None):
        ...
        loss_giou = 1 - paired_generalized_box_iou_xyxy(
            box_cxcywh_to_xyxy(src_boxes), box_cxcywh_to_xyxy(target_boxes)
        )
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        if self.decoder_moe_balance_weight > 0 and 'moe_load_balance_loss' in outputs:
            moe_weight = 0.0 if current_epoch < self.moe_balance_warmup_epochs else self.decoder_moe_balance_weight
            if moe_weight > 0:
                losses['loss_moe_balance'] = outputs['moe_load_balance_loss'] * moe_weight

        if self.cass_loss_weight > 0 and targets:
            ...
            if encoder_info:
                ...
                if importance_scores_list and feat_shapes_list:
                    pred_scores = importance_scores_list[0]
                    feat_shape = feat_shapes_list[0]
                    if pred_scores is not None:
                        img_h, img_w = None, None
                        if targets and 'size' in targets[0]:
                            size = targets[0]['size']
                            img_h, img_w = int(size[0].item()), int(size[1].item())
                        elif targets and 'orig_size' in targets[0]:
                            size = targets[0]['orig_size']
                            img_h, img_w = int(size[0].item()), int(size[1].item())
                        if img_h is not None and img_w is not None:
                            gt_bboxes = self._build_absolute_xyxy_boxes(targets, (img_h, img_w))
                            pruner = outputs.get('shared_token_pruner', None) if isinstance(outputs, dict) else None
                            if pruner is not None and getattr(pruner, 'use_cass', False):
                                cass_loss = pruner.compute_cass_loss_from_info(
                                    info={'token_importance_scores': pred_scores},
                                    gt_bboxes=gt_bboxes,
                                    feat_shape=feat_shape,
                                    img_shape=(img_h, img_w),
                                )
                                losses['loss_cass'] = cass_loss * self.cass_loss_weight
    ...
    def get_loss_meta_info(self, loss, outputs, targets, indices):
        ...
        if self.boxes_weight_format == 'iou':
            iou = paired_box_iou_xyxy(
                box_cxcywh_to_xyxy(src_boxes.detach()), box_cxcywh_to_xyxy(target_boxes)
            )
        elif self.boxes_weight_format == 'giou':
            iou = paired_generalized_box_iou_xyxy(
                box_cxcywh_to_xyxy(src_boxes.detach()), box_cxcywh_to_xyxy(target_boxes)
            )
        else:
    ...
    @staticmethod
    def _build_absolute_xyxy_boxes(targets, img_shape):
        gt_bboxes = []
        ...
        for target in targets:
            ...
            boxes_abs[:, 0] = (cx - w / 2) * img_w
            boxes_abs[:, 1] = (cy - h / 2) * img_h
            boxes_abs[:, 2] = (cx + w / 2) * img_w
            boxes_abs[:, 3] = (cy + h / 2) * img_h
