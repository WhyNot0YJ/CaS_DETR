    ...
    def __getitem__(self, idx):
        ...
        if self.use_ignore_regions:
            ...
            for key in ('boxes', 'labels', 'area', 'iscrowd', 'masks', 'keypoints',
                        'occluded_state', 'truncated_state'):
...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image: Image.Image, target, **kwargs):
        ...
        has_occ = any("occluded_state" in obj for obj in anno)
        has_trunc = any("truncated_state" in obj for obj in anno)
        occ_tensor = trunc_tensor = None
        if has_occ or has_trunc:
            occ_tensor = torch.tensor(
                [float(obj.get("occluded_state", 0.0)) for obj in anno], dtype=torch.float32
            )[keep]
            trunc_tensor = torch.tensor(
                [float(obj.get("truncated_state", 0.0)) for obj in anno], dtype=torch.float32
            )[keep]
        ...
        if occ_tensor is not None:
            target["occluded_state"] = occ_tensor
        if trunc_tensor is not None:
            target["truncated_state"] = trunc_tensor
